...
const arbAction: fc.Arbitrary<Action> = fc.oneof(
  ...
  { weight: 2, arbitrary: fc.record({ type: fc.constant("startEdit" as const), draft: fc.option(fc.string({ maxLength: 8 }), { nil: undefined }) }) },
  { weight: 1, arbitrary: fc.record({ type: fc.constant("updateDraft" as const), draft: fc.string({ maxLength: 8 }) }) },
  { weight: 2, arbitrary: fc.constant({ type: "commitEdit" } as Action) },
  { weight: 1, arbitrary: fc.constant({ type: "cancelEdit" } as Action) },
  { weight: 2, arbitrary: fc.record({ type: fc.constant("insertRow" as const), index: arbIndex }) },
  { weight: 2, arbitrary: fc.record({ type: fc.constant("insertCol" as const), index: arbIndex }) },
  { weight: 2, arbitrary: fc.record({ type: fc.constant("deleteRow" as const), id: arbRowId }) },
  { weight: 2, arbitrary: fc.record({ type: fc.constant("deleteCol" as const), id: arbColId }) },
